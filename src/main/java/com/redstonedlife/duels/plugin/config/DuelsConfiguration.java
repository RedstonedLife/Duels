package com.redstonedlife.duels.plugin.config;

import com.redstonedlife.duels.api.InvalidWorldException;
import com.redstonedlife.duels.plugin.Duels;
...
import com.redstonedlife.duels.plugin.config.processors.DeleteOnEmptyProcessor;
import com.redstonedlife.duels.plugin.config.serializers.BigDecimalTypeSerializer;
import com.redstonedlife.duels.plugin.config.serializers.CommandCooldownSerializer;
import com.redstonedlife.duels.plugin.config.serializers.LocationTypeSerializer;
import com.redstonedlife.duels.plugin.config.serializers.MaterialTypeSerializer;
...
public class DuelsConfiguration {
    ...
    private Class<?> resourceClass = DuelsConfiguration.class;
    ...
    public DuelsConfiguration(final File configFile) {
        this(configFile, null);
    }

    public DuelsConfiguration(final File configFile, final String templateName) {
        this(configFile, templateName, (String) null);
    }

    public DuelsConfiguration(final File configFile, final String templateName, final Class<?> resourceClass) {
        this(configFile, templateName, (String) null);
        this.resourceClass = resourceClass;
    }

    public DuelsConfiguration(final File configFile, final String templateName, final String header) {
    ...
    public void setRootHolder(final Class<?> holderClass, final Object holder) {
        try {
            ...
            Duels.getWrappedLogger().log(Level.SEVERE, "Error while saving user config: " + configFile.getName(), e);
    ...
    public Map<String, LazyLocation> getLocationSectionMap(final String path) {
        ...
        for (final Map.Entry<String, CommentedConfigurationNode> entry : ConfigurateUtil.getMap(node).entrySet()) {
            ...
            try {
                ...
                Duels.getWrappedLogger().log(Level.WARNING, "Error serializing key " + entry.getKey(), e);
            }
        }
        return result;
    }
    ...
    public <T> void setExplicitList(final String path, final List<T> list, final Type type) {
        try {
            ...
            Duels.getWrappedLogger().log(Level.SEVERE, e.getMessage(), e);
        }
    }
    ...
    public synchronized void load() {
        if (pendingWrites.get() != 0) {
            Duels.getWrappedLogger().log(Level.INFO, "Parsing config file {0} has been aborted due to {1} current pending write(s).", new Object[]{configFile, pendingWrites.get()});
            return;
        }

        if (configFile.getParentFile() != null && !configFile.getParentFile().exists()) {
            if (!configFile.getParentFile().mkdirs()) {
                Duels.getWrappedLogger().log(Level.SEVERE, tl(null,"failedToCreateConfig", configFile.toString()));
                return;
            }
        }
        ...
        if (!configFile.exists()) {
            if (legacyFileExists()) {
                ...
                try (final InputStream is = resourceClass.getResourceAsStream(templateName)) {
                    Duels.getWrappedLogger().log(Level.INFO, tl(null,"creatingConfigFromTemplate", configFile.toString()));
                    Files.copy(is, configFile.toPath());
                } catch (IOException e) {
                    Duels.getWrappedLogger().log(Level.SEVERE, tl(null,"failedToWriteConfig", configFile.toString()), e);
                }
            }
        }
        ...
        try {
            ...
            if (configFile.renameTo(broken)) {
                Duels.getWrappedLogger().log(Level.SEVERE, "The file " + configFile + " is broken, it has been renamed to " + broken, e.getCause());
                return;
            }
            Duels.getWrappedLogger().log(Level.SEVERE, "The file " + configFile + " is broken. A backup file has failed to be created", e.getCause());
     ...
     * <p>
     * A transaction informs Duels to pause the saving of data. This is should be used when
